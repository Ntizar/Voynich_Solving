...
# Ensure pure ascii/utf-8 prints for windows
...
with open('zenodo_voynich/corpus/voynich_sta.txt', 'r', encoding='utf-8') as f:
    for line in f:
        if line.startswith('<f1r.') and '>' in line:
            ...
            if len(lines_data) >= 8: # Take first 8 lines
                break
...
header = "| L | Col 1 | Col 2 | Col 3 | Col 4 | Col 5 | Col 6 | Col 7 | Col 8 | Col 9 | Col 10 |"
separator = "|---|---|---|---|---|---|---|---|---|---|---|"
print(header)
print(separator)
    ...
    for j in range(max_words):
        if j < len(row):
            ...
            if i > 0 and j < len(matrix[i-1]):
                ...
                if suf and prev_suf and suf == prev_suf:
                    suf_display = f"-->[ {suf} ]<--" # Pure ASCII highlight
